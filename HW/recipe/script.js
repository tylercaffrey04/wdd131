...
function renderRecipes(recipeArray) {
    ...
    if (recipeArray.length === 0) {
        container.innerHTML = '<p class="no-results">No recipes found. Try searching for something else!</p>';
        return;
    }
    ...
    recipeArray.forEach(recipe => {
        ...
        const tagText = recipe.tags && recipe.tags.length > 0 ? recipe.tags[0].toLowerCase() : 'recipe';
...
function displayInitialRandomRecipe() {
...
document.querySelector('.search-input').addEventListener('input', handleSearchAndSort);
document.querySelector('.search-btn').addEventListener('click', handleSearchAndSort);
